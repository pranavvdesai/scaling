...
const Modal = ({
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      ...
      console.log(parseInt(tasksData.stakedAmount * 100000));
      ...
      let transaction = await token.burn(
        address,
        parseInt(tasksData.stakedAmount * 100000)
      );
          ...
          <div className="relative p-3 flex-auto">
            <form className="bg-transparent shadow-md rounded px-8 pt-3 pb-8 w-full">
              ...
              <input
                ...
                required
                value={tasksData.taskName}
                onChange={(e) =>
                  setTasksData({ ...tasksData, taskName: e.target.value })
                }
              />
              ...
              <input
                ...
                className="shadow appearance-none border rounded w-full text-white
                                       block h-10 bg-[#ffffff12] text-white rounded-lg px-2 border border-slate-600 py-5 mt-2 mb-2 mr-10 text-sm w-full focus:outline-none
                                        transition transform duration-100 ease-out"
                value={tasksData.stakedAmount}
                onChange={(e) =>
                  setTasksData({ ...tasksData, stakedAmount: e.target.value })
                }
                required
                min={0}
                max={100}
              ...
              <input
                ...
                className="shadow appearance-none border rounded w-full text-white
                                       block h-10 bg-[#ffffff12] text-white rounded-lg px-2 border border-slate-600 py-5 mt-2 mb-2 mr-10 text-sm w-full focus:outline-none
                                        transition transform duration-100 ease-out"
                value={tasksData.taskDuration}
                onChange={(e) =>
                  setTasksData({ ...tasksData, taskDuration: e.target.value })
                }
                required
                min={0}
                max={10}
